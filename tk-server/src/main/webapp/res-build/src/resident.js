...
define(function(require, exports, module) {
    ...
    var listTpl = juicer(
        [
            ...
            '{@if index%2==0}',
            '<tr role="row" class="odd" data-residentid="${item.id}">',
            '{@else}',
            '<tr role="row" class="even" data-residentid="${item.id}">',
            '{@/if}',
            '    <td>${item.id}</td>',
            '    <td>${item.realname}</td>',
            ...
            '        <a href="' + ROOTPAth + '/admin/bussiness/resident/updateView?id=${item.id}&currentpage=${item.currentpage}&pcode=3&subcode=1" class="btn btn-default btn-xs j-edit" ><span class="iconfont iconfont-xs">&#xe62d;</span>查看</a> ',
            //删除
    ...
    var Utilitiy = {
        init: function() {
            ...
            pageIndex = new Page({
                ajax: {
                    url: ROOTPAth + '/admin/bussiness/resident/list',
                    type: 'POST',
                    dataType: 'json',
                    data: function() {
        ...
        delitem: function($that) {
            ...
            var residentid = $tr.data("residentid");
            var delPath = ROOTPAth + '/admin/bussiness/resident/delete/';
            $.ajax({
                url: delPath,
                type: "POST",
                data: {
