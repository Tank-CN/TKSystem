...
define(function(require, exports, module) {
    ...
    var Utilitiy = {
        ...
        initForm: function() {
            ...
            $.ajax({
                type: "POST",
                url: ROOTPAth + "/admin/bussiness/sign/detail/",
                dataType: "json",
                data: {
                    id: signId
                },
                success: function(data) {
                    ...
                    var applysigndate_val = $("#applysigndate").val();
                    if (applysigndate_val) {
                        $("#applysigndate").val(formatDate(parseInt(applysigndate_val), "2016-05-31"));
                    }
                    var signdate_val = $("#signdate").val();
                    if (signdate_val) {
                        $("#signdate").val(formatDate(parseInt(signdate_val), "2016-05-31"));
                    }
                    var expiredate_val = $("#expiredate").val();
                    if (expiredate_val) {
                        $("#expiredate").val(formatDate(parseInt(expiredate_val), "2016-05-31"));
                    }
                    var applycanceldate_val = $("#applycanceldate").val();
                    if (applycanceldate_val) {
                        $("#applycanceldate").val(formatDate(parseInt(applycanceldate_val), "2016-05-31"));
                    }
                    var canceldate_val = $("#canceldate").val();
                    if (canceldate_val) {
                        $("#canceldate").val(formatDate(parseInt(canceldate_val), "2016-05-31"));
                    }
                    var verifyapplydate_val = $("#verifyapplydate").val();
                    if (verifyapplydate_val) {
                        $("#verifyapplydate").val(formatDate(parseInt(verifyapplydate_val), "2016-05-31"));
                    }
                    var verifycancelddate_val = $("#verifycancelddate").val();
                    if (verifycancelddate_val) {
                        $("#verifycancelddate").val(formatDate(parseInt(verifycancelddate_val), "2016-05-31"));
                    }
        ...
        bind: function() {
            ...
            $addForm.validate({
                rules: {
                    ...
                    //     required: true,
                    //     remote: { //自带远程验证存在的方法
                    //         url: ROOTPAth + '/admin/bussiness/sign/code/check',
                    //         type: "GET",
                    //         dataType: "json",
                    //         data: {
                    //             /*username: function () {
                ...
                submitHandler: function(fm) {
                    ...
                    $(fm).ajaxSubmit({
                        type: "POST",
                        dataType: "html",
                        url: ROOTPAth + "/admin/bussiness/sign/saveorupdate",
                        data: {
                            id: signId
                        },
                        ...
                        success: function(data) {
                            ...
                            if (newdata.code === 1) {
                                ...
                                $tipModal.on('show.bs.modal', function(event) {
                                    $tipModal.find(".j-modal-closebtn").attr("href", ROOTPAth + "/admin/bussiness/sign/updateView?id=" + signId + "&pcode=3&subcode=2&scurrentpage=" + scurrentpage)
                                });
                                $tipModal.modal("show");
                            }
                        },
        ...
        createTree: function() {
            ...
            //         $addForm.find('input[name="streetid"]').val(obj.longcode.substring(0, 8));
            //         $addForm.find('input[name="villageid"]').val(obj.longcode.substring(0, 10));
